...
let queryLoki, queryLokiCount, debugSample;
try {
    const lokiService = require('./lokiService');
    queryLoki = lokiService.queryLoki;
    queryLokiCount = lokiService.queryLokiCount;
    debugSample = lokiService.debugSample;
} catch (e) {
    console.warn('[KPI] lokiService introuvable:', e.message);
    queryLoki = async() => 0;
    queryLokiCount = async() => 0;
...
async function getKpis(userId, tenantId) {
    ...
    // ── Debug Loki : voir le format brut des logs ─────────────────────────────
    // RETIRE CES 2 LIGNES une fois que les logs KPI fonctionnent
    await debugSample(tenantName);
    // ─────────────────────────────────────────────────────────────────────────
